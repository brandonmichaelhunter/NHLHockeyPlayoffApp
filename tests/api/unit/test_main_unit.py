...
async def test_read_nhl_scores_with_valid_date(httpx_mock: HTTPXMock):
    ...
    try:
        ...
        # Assert the response
        assert response.status_code == 200, (
            "Expected a 200 OK response for a valid date"
        )
        assert response.json() == mockNHLScores, (
            "Expected the mocked NHL scores to be returned in the response"
        )
    finally:
        main_module.app.dependency_overrides.clear()
...
async def test_read_nhl_scores_with_invalid_date(invalid_date, expected_payload):
    ...
    def exec_side_effect(stmt):
        ...
        selected_date = (
            params.get("date_1")
            or params.get("nhlScoreDateSelect_1")
    ...
    try:
        ...
        async with AsyncClient(
            ...
            response = await ac.get(
                "/nhl_scores",
                params={"nhlScoreDateSelect": invalid_date},
                headers={"hx-request": "false"},
            )
        ...
        assert response.json() == expected_payload, (
            "Expected the mocked NHL scores not to be returned in the response"
        )
    finally:
        main_module.app.dependency_overrides.clear()
    ...
    try:
        ...
        with (
            patch.object(
                ...
                ),  # Represents what the mock returns whenenver templates.TemplateResponse is called.
            ) as template_mock
        ):
            async with AsyncClient(
                transport=ASGITransport(app=main_module.app),
    ...
    fake_nhl_schedule = [
        {
            ...
            "awayTeamLogoUrl": "https://assets.nhle.com/logos/nhl/svg/CAR_light.svg",
            "homeTeamName": "Vegas Golden Knights",
            "homeScore": 5,
            "homeTeamNameAbbrv": "VGK",
            "homeTeamLogoUrl": "https://assets.nhle.com/logos/nhl/svg/VGK_light.svg",
            ...
            "goalieLastName": "Hart",
            "goalieHeadShotUrl": "https://assets.nhle.com/mugs/nhl/20252026/VGK/8479394.png",
            "winningGoalScorerPlayerID": 8477447,
            "skaterFirstName": "Shea",
            "skaterLastName": "Theodore",
            "skaterHeadShotUrl": "https://assets.nhle.com/mugs/nhl/20252026/VGK/8477447.png",
...
async def test_get_nhl_teams_returns_valid_nhl_teams(mock_function):
    ...
    # Mock the nhl_scores fake data.
    fake_nhl_teams = [
        {"id": 30, "team_name": "Carolina Hurricanes"},
        {"id": 54, "team_name": "Vegas Golden Knights"},
    ]
    ...
    try:
        ...
        assert kwargs["name"] == "nhl_teams_options.html", (
            "Expected the nhl_teams_options.html template to be rendered"
        )
        assert kwargs["context"]["teams"] == fake_nhl_teams, (
            "Expected the mocked NHL teams to be passed in the context of the template response"
        )
...
async def test_get_nhl_stats_returns_stats_leaders_template(
    ...
    try:
        ...
        mock_get_nhl_goal_leaders.assert_called_once_with(
            session=mock_session, TeamName=7
        )
        mock_get_nhl_plusminus_leaders.assert_called_once_with(
            session=mock_session, TeamName=7
        )
        mock_get_nhl_points_leaders.assert_called_once_with(
            session=mock_session, TeamName=7
        )
