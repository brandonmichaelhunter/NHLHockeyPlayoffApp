...
class nhl_etl_manager:
    ...
    def __init__(self, apiClient, dbManager, displayConsoleLogs: bool = True):
        ...
        self._logWrapper = utility_manager(
            "nhl_etl_manager", "nhl_etl_manager.log", 10, self._displayConsoleLogs
        )
    ...
    def provision_app_tables(self):
        try:
            query_methods = [
                self.__get_nhl_teams_query,
                self.__get_nhl_goals_leaders_query,
                self.__get_nhl_plus_minus_leaders_query,
                self.__get_nhl_save_percentage_leaders_query,
                self.__get_nhl_goalie_wins_leaders_query,
                self.__get_nhl_schedule_games_query,
            ...
            self._log.error(
                f"provision_app_tables: An error occurred while provisioning app tables: {e}"
            )
            raise Exception(f"An error occurred while provisioning app tables: {e}")

    # ---------------------------------
    def __get_nhl_teams_query(self):
        query = """
    ...
    def __get_nhl_goals_leaders_query(self):
        query = """
    ...
    def __get_nhl_plus_minus_leaders_query(self):
        query = """
    ...
    def __get_nhl_save_percentage_leaders_query(self):
        query = """
    ...
    def __get_nhl_goalie_wins_leaders_query(self):
    ...
    def __get_nhl_schedule_games_query(self):
    ...
    def __get_game_stats_query(self) -> str:
        query = """
            CREATE TABLE IF NOT EXISTS game_stats (
                ...
                plus_minus INTEGER,
                assists INTEGER,
                total_penalities_minutes INTEGER
            );
            """
        return query
    ...
    def __get_nhl_playoff_schedule_games_query(self) -> str:
        query = """
            CREATE TABLE IF NOT EXISTS nhl_playoff_schedule_games (
                        ...
                        topSeedsWin TEXT,
                        topSeedsTeamID INTEGER,
                        bottomSeedsWin TEXT,
                        bottomSeedsTeamID INTEGER,
                        seriesTitle TEXT,
                        round TEXT,
                        stationInfo TEXT,
                        gameNumberOfSeries TEXT,
                        winningGoaliePlayerID INTEGER,
                        winningGoalScorerPlayerID INTEGER,
                        venueName TEXT,
                        periods INTEGER,
                        series_info TEXT,
                        DateCreated TIMESTAMP WITH TIME ZONE DEFAULT now()
    ...
    def register_api_urls(self) -> list:
        try:
            ...
            api_urls.append(
                api_url_request(
                    function_name="teams_info", endpoint_url="stats/rest/en/team"
                )
            )
            api_urls.append(
                api_url_request(
                    function_name="team_roster",
                    endpoint_url="v1/roster/{team_abbrv}/current",
                )
            )
            api_urls.append(
                api_url_request(
                    function_name="player_info",
                    endpoint_url="v1/player/{player_id}/landing",
                )
            )
            api_urls.append(
                api_url_request(
                    function_name="game_box_score",
                    endpoint_url="v1/player/{player}/game-log/{season}/{game_type}",
                )
            )
            api_urls.append(
                api_url_request(function_name="seasons", endpoint_url="v1/season")
            )

            return api_urls
        except Exception as e:
            self._log.error(
                f"register_api_urls: An error occurred while registering API URLs: {e}"
            )
    ...
    def run_data_extraction_process(self):
        try:
            ...
            self._log.error(
                f"run_data_extraction_process: An error occurred while running the data extraction process: {e}"
            )
            raise Exception(
                f"An error occurred while running the data extraction process: {e}"
            )
    ...
    def run_seasons_pipeline(self):
        try:
            ...
            self._log.error(
                f"run_seasons_pipeline: An error occurred while running the seasons pipeline: {e}"
            )
            raise Exception(
                f"An error occurred while running the seasons pipeline: {e}"
            )
    ...
    def extract_seasons_info(self) -> list:
        try:
            extracted_data = self._apiClient.fetch_nhl_data(
                api_url_request(
                    function_name="seasons", endpoint_url="v1/season"
                ).endpoint_url
            )
            return extracted_data
        except Exception as e:
            self._log.error(
                f"extract_seasons_info: An error occurred while extracting seasons info: {e}"
            )
    ...
    def transform_seasons_info(self, extractedData: list) -> list:
        try:
            ...
            self._log.error(
                f"transform_seasons_info: An error occurred while transforming seasons info: {e}"
            )
    ...
    def load_seasons_info(self, transformedData: list):
        try:
            ...
            for seasonModel in transformedData:
                ...
                """.format(
                    id=seasonModel.id,
                    start_season_year=seasonModel.startYear,
                    end_season_year=seasonModel.endYear,
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert season data for season ID: {seasonModel.id}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Season data inserted successfully for season ID: {seasonModel.id}"
                    )
        except Exception as e:
            self._log.error(
                f"load_seasons_info: An error occurred while loading seasons info: {e}"
            )
    ...
    def run_teams_pipeline(self):
        try:
            ...
            self._log.error(
                f"run_teams_pipeline: An error occurred while running the teams pipeline: {e}"
            )
    ...
    def extract_teams_info(self) -> list:
        try:
            ...
            self._log.error(
                f"extract_teams_info: An error occurred while extracting teams info: {e}"
            )
    ...
    def extract_team_franchise_info(self) -> list:
        try:
            ...
            self._log.error(
                f"extract_team_franchise_info: An error occurred while extracting team franchise info: {e}"
            )
            raise Exception(
                f"An error occurred while extracting team franchise info: {e}"
            )
        ...
        try:
            ...
            for playoff_franchise_team in playoff_francise_teams:
                ...
                for item in teamData["data"]:
                    if franchise_team_id == item["franchiseId"] and (
                        item["fullName"] != "Quebec Nordiques"
                        and item["fullName"] != "Hartford Whalers"
                        and item["fullName"] != "Minnesota North Stars"
                        and item["fullName"] != "Utah Hockey Club"
                    ):
                        id = item["id"]
                        franchise_id = item["franchiseId"]
                        name = item["fullName"]
                        abbrv = item["triCode"]
                        logoURL = f"https://assets.nhle.com/logos/nhl/svg/{abbrv}_light.svg".format(
                            abbrv=abbrv
                        )
                        city = ""
                        state = ""
                        country = ""
                        teamModel = nhl_team(
                            id, franchise_id, name, abbrv, logoURL, city, state, country
                        )
                        transformed_data.append(teamModel)
            return transformed_data
        except Exception as e:
            self._log.error(
                f"transform_teams_info: An error occurred while transforming teams info: {e}"
            )
    ...
    def load_teams_info(self, transformedData: list):
        try:
            ...
            for teamModel in transformedData:
                ...
                """.format(
                    id=teamModel.id,
                    franchise_id=teamModel.franchise_id,
                    name=teamModel.name.replace("'", "''"),
                    abbrv=teamModel.abbrv.replace("'", "''"),
                    logoURL=teamModel.logoURL.replace("'", "''"),
                    city=teamModel.city.replace("'", "''"),
                    state=teamModel.state.replace("'", "''"),
                    country=teamModel.country.replace("'", "''"),
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert team data for team ID: {teamModel.id}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Team data inserted successfully for team ID: {teamModel.id}"
                    )
        except Exception as e:
            self._log.error(
                f"load_teams_info: An error occurred while loading teams info: {e}"
            )
    ...
    def run_team_roster_pipeline(self):
        try:
            ...
            self._log.error(
                f"run_team_roster_pipeline: An error occurred while running the team roster pipeline: {e}"
            )
            raise Exception(
                f"An error occurred while running the team roster pipeline: {e}"
            )
    ...
    def load_team_roster_info(self, transformedData: list):
        try:
            ...
            for teamRosterModel in transformedData:
                ...
                """.format(
                    team_id=teamRosterModel.teamID,
                    player_id=teamRosterModel.playerID,
                    start_date=teamRosterModel.startDate,
                    end_date=teamRosterModel.endDate,
                    jersey_number=teamRosterModel.jerseyNumber,
                    position=teamRosterModel.position.replace("'", "''"),
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert team roster data for team ID: {teamRosterModel.teamID} and player ID: {teamRosterModel.playerID}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Team roster data inserted successfully for team ID: {teamRosterModel.teamID} and player ID: {teamRosterModel.playerID}"
                    )
        except Exception as e:
            self._log.error(
                f"load_team_roster_info: An error occurred while loading team roster info: {e}"
            )
    ...
    def load_players_info(self, transformedData: list):
        try:
            ...
            for playerModel in transformedData:
                ...
                """.format(
                    id=playerModel.id,
                    first_name=playerModel.first_name.replace("'", "''"),
                    m_initial="",
                    last_name=playerModel.last_name.replace("'", "''"),
                    age=0,
                    birth_place=playerModel.birth_date.replace("'", "''"),
                    headshot_url=playerModel.headshot_url.replace("'", "''"),
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert player data for player ID: {playerModel.id}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Player data inserted successfully for player ID: {playerModel.id}"
                    )
        except Exception as e:
            self._log.error(
                f"load_players_info: An error occurred while loading players info: {e}"
            )
        ...
        try:
            ...
            for teamRoster in teamsRosters:
                ...
                for playerRoster in teamRoster.roster["forwards"]:
                    ...
                    if (
                        "sweaterNumber" in playerRoster
                        and playerRoster["sweaterNumber"] is not None
                    ):
                    ...
                    teamPlayerModel = teamroster(
                        None, teamID, playerID, None, None, jerseyNumber, position
                    )
            ...
            self._log.error(
                f"transform_team_roster_info: An error occurred while transforming team roster info: {e}"
            )
            raise Exception(
                f"An error occurred while transforming team roster info: {e}"
            )
    ...
    def run_games_pipeline(self):
        try:
            ...
            jsonData = self.extract_games_info(
            ...
            self._log.error(
                f"run_games_pipeline: An error occurred while running the games pipeline: {e}"
            )
    ...
    def extract_games_info(self, url: str):
        try:
            ...
            self._log.error(
                f"extract_games_info: An error occurred while extracting games info: {e}"
            )
    ...
    def transform_games_info(self, extractedData: list) -> list:
        try:
            ...
            self._log.error(
                f"transform_games_info: An error occurred while transforming games info: {e}"
            )
    ...
    def load_games_info(self, transformedData: list):
        try:
            ...
            for gameModel in transformedData:
                ...
                """.format(
                    id=gameModel.id,
                    gameDate=gameModel.gameDate.replace("'", "''"),
                    gameNumber=gameModel.gameNumber,
                    gameScheduleStateId=gameModel.gameScheduleStateId,
                    gameStateId=gameModel.gameStateId,
                    gameTypeId=gameModel.gameTypeId,
                    homeScore=gameModel.homeScore,
                    homeTeamId=gameModel.homeTeamId,
                    period=gameModel.period,
                    seasonId=gameModel.seasonId,
                    visitingScore=gameModel.visitingScore,
                    visitingTeamId=gameModel.visitingTeamId,
                    year=gameModel.year,
                    month=gameModel.month,
                    day=gameModel.day,
                )
                print(query)
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert game data for game ID: {gameModel.id}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Game data inserted successfully for game ID: {gameModel.id}"
                    )

        except Exception as e:
            self._log.error(
                f"load_games_info: An error occurred while loading games info: {e}"
            )
    ...
    def run_game_player_stats_pipeline(self):
        try:
            ...
            self._log.error(
                f"run_game_player_stats_pipeline: An error occurred while running the game player stats pipeline: {e}"
            )
            raise Exception(
                f"An error occurred while running the game player stats pipeline: {e}"
            )
    ...
    def extract_game_player_stats_info(self, url: str):
        try:
            ...
            self._log.error(
                f"extract_game_player_stats_info: An error occurred while extracting game player stats info: {e}"
            )
            raise Exception(
                f"An error occurred while extracting game player stats info: {e}"
            )
        ...
        try:
            ...
            for game in games:
                ...
                if "playerByGameStats" not in game or game["playerByGameStats"] is None:
                    self._log.warning(
                        f"No player game stats data found for game ID: {gameId}"
                    )
                    continue
                ...
                for homeTeam in game["playerByGameStats"]["homeTeam"]["forwards"]:
                    ...
                    if (
                        "faceoffWinningPctg" in homeTeam
                        and homeTeam["faceoffWinningPctg"] is not None
                    ):
            ...
            self._log.error(
                f"transform_game_player_stats_info: An error occurred while transforming game player stats info: {e}"
            )
            raise Exception(
                f"An error occurred while transforming game player stats info: {e}"
            )
        ...
        try:
            ...
            for playerGameStatsModel in transformedData:
                ...
                """.format(
                    ...
                    gameTypeID=playerGameStatsModel.gameTypeID,
                    playerID=playerGameStatsModel.playerID,
                    position=playerGameStatsModel.position.replace("'", "''"),
                    goals=playerGameStatsModel.goals,
                    assists=playerGameStatsModel.assists,
                    points=playerGameStatsModel.points,
                    plusMinus=playerGameStatsModel.plusMinus,
                    ...
                    evenStrengthShotsAgainst=str(
                        playerGameStatsModel.evenStrengthShotsAgainst
                    ).replace("'", "''"),
                    powerPlayShotsAgainst=str(
                        playerGameStatsModel.powerPlayShotsAgainst
                    ).replace("'", "''"),
                    shorthandedShotsAgainst=str(
                        playerGameStatsModel.shorthandedShotsAgainst
                    ).replace("'", "''"),
                    saveShotsAgainst=str(playerGameStatsModel.saveShotsAgainst).replace(
                        "'", "''"
                    ),
                    savePctg=playerGameStatsModel.savePctg,
                    evenStrengthGoalsAgainst=playerGameStatsModel.evenStrengthGoalsAgainst,
                    powerPlayGoalsAgainst=playerGameStatsModel.powerPlayGoalsAgainst,
                    shorthandedGoalsAgainst=playerGameStatsModel.shorthandedGoalsAgainst,
                    goalsAgainst=playerGameStatsModel.goalsAgainst,
                    shotsAgainst=playerGameStatsModel.shotsAgainst,
                    saves=playerGameStatsModel.saves,
                    toi=playerGameStatsModel.toi.replace("'", "''"),
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert player game stats data for player ID: {playerGameStatsModel.playerID} and game ID: {playerGameStatsModel.gameID}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Player game stats data inserted successfully for player ID: {playerGameStatsModel.playerID} and game ID: {playerGameStatsModel.gameID}"
                    )
        except Exception as e:
            self._log.error(
                f"load_game_player_stats_info: An error occurred while loading game player stats info: {e}"
            )
            raise Exception(
                f"An error occurred while loading game player stats info: {e}"
            )
    ...
    def run_game_box_score_pipeline(self):
        try:
            # get game ids from games table.
            game_dates = self._dbManager.execute_fetch(
                f"select  distinct gameDate from games where year={self.__getCurrentYear()} and month in (4,5,6);"
            )

            # extract game player stats data from NHL API
            jsonDataList = []
            for game_date in game_dates:
                jsonData = self.extract_game_box_scores_info(
                    f"https://api-web.nhle.com/v1/score/{game_date[0]}"
                )
                if jsonData is not None:
                    jsonDataList.append(jsonData)
                else:
                    self._log.warning(
                        f"No game box scores data found for game date: {game_date[0]}"
                    )
            # transform json data into list of models
            ...
            self._log.error(
                f"run_game_box_score_pipeline: An error occurred while running the game box scores pipeline: {e}"
            )
            raise Exception(
                f"An error occurred while running the game box scores pipeline: {e}"
            )
    ...
    def extract_game_box_scores_info(self, url: str):
        try:
            ...
            self._log.error(
                f"extract_game_box_scores_info: An error occurred while extracting game box scores info: {e}"
            )
            raise Exception(
                f"An error occurred while extracting game box scores info: {e}"
            )
    ...
    def transform_game_box_scores_info(self, gameBoxScores: list) -> list:
        try:
            ...
            for gameBoxScore in gameBoxScores:
                for game in gameBoxScore["games"]:
                    ...
                    for goal in game["goals"]:
                        ...
                        gameboxscoreModel = gameboxscore(
                            id=None,
                            gameID=gameId,
                            period=period,
                            awayScore=awayScore,
            ...
            self._log.error(
                f"transform_game_box_scores_info: An error occurred while transforming game box scores info: {e}"
            )
            raise Exception(
                f"An error occurred while transforming game box scores info: {e}"
            )
    ...
    def load_game_box_scores_info(self, transformedData: list):
        try:
            ...
            for gameboxscoreModel in transformedData:
                ...
                """.format(
                    gameID=gameboxscoreModel.gameID,
                    period=gameboxscoreModel.period,
                    awayScore=gameboxscoreModel.awayScore,
                    homeScore=gameboxscoreModel.homeScore,
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert game box scores data for game ID: {gameboxscoreModel.gameID}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Game box scores data inserted successfully for game ID: {gameboxscoreModel.gameID}"
                    )
        except Exception as e:
            self._log.error(
                f"load_game_box_scores_info: An error occurred while loading game box scores info: {e}"
            )
            raise Exception(
                f"An error occurred while loading game box scores info: {e}"
            )
    ...
    def run_goaltending_win_leaders_pipeline(self):
        try:
            ...
            jsonData = self.extract_goaltending_win_leaders_info(
                f"https://api-web.nhle.com/v1/goalie-stats-leaders/{season}/3?categories=wins&limit=100"
            )
            ...
            self._log.error(
                f"run_goaltending_win_leaders_pipeline: An error occurred while running the goaltending win leaders pipeline: {e}"
            )
            raise Exception(
                f"An error occurred while running the goaltending win leaders pipeline: {e}"
            )
    ...
    def extract_goaltending_win_leaders_info(self, url: str):
        try:
            ...
            self._log.error(
                f"extract_goaltending_win_leaders_info: An error occurred while extracting goaltending win leaders info: {e}"
            )
            raise Exception(
                f"An error occurred while extracting goaltending win leaders info: {e}"
            )
    ...
    def transform_goaltending_win_leaders_info(self, extractedData: list) -> list:
        try:
            ...
            for item in extractedData["wins"]:
                ...
                wins = item["value"]
                goaltendingWinLeaderModel = nhl_goaltending_win_leader(
                    player_id=playerID,
                    team_id=team_id,
                    seasons=season,
            ...
            self._log.error(
                f"transform_goaltending_win_leaders_info: An error occurred while transforming goaltending win leaders info: {e}"
            )
            raise Exception(
                f"An error occurred while transforming goaltending win leaders info: {e}"
            )
        ...
        try:
            ...
            for goaltendingWinLeaderModel in transformedData:
                ...
                """.format(
                    ...
                    team_id=goaltendingWinLeaderModel.team_id,
                    seasons=goaltendingWinLeaderModel.seasons,
                    first_name=goaltendingWinLeaderModel.first_name.replace("'", "''"),
                    last_name=goaltendingWinLeaderModel.last_name.replace("'", "''"),
                    team_name=goaltendingWinLeaderModel.team_name.replace("'", "''"),
                    team_abbrv=goaltendingWinLeaderModel.team_abbrv.replace("'", "''"),
                    wins=goaltendingWinLeaderModel.wins,
                    headshot_url=goaltendingWinLeaderModel.headshot_url.replace(
                        "'", "''"
                    ),
                    teamlogo_url=goaltendingWinLeaderModel.teamlogo_url.replace(
                        "'", "''"
                    ),
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert goaltending win leader data for player: {goaltendingWinLeaderModel.first_name} {goaltendingWinLeaderModel.last_name}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"Goaltending win leader data inserted successfully for player: {goaltendingWinLeaderModel.first_name} {goaltendingWinLeaderModel.last_name}"
                    )
        except Exception as e:
            self._log.error(
                f"load_goaltending_win_leaders_info: An error occurred while loading goaltending win leaders info: {e}"
            )
            raise Exception(
                f"An error occurred while loading goaltending win leaders info: {e}"
            )
    ...
    def run_nhl_scores_pipeline(self):
        try:
            ...
            for nhlGameDate in nhlGameDates:
                jsonData = self.extract_nhl_scores_info(
                    f"https://api-web.nhle.com/v1/score/{nhlGameDate}"
                )
                if jsonData is not None:
                    nhlGameScores.append(jsonData)
                else:
                    self._log.warning(
                        f"No NHL scores data found for game date: {nhlGameDate}"
                    )
            transformedData = self.transform_nhl_scores_info(nhlGameScores)
            self.load_nhl_scores_info(transformedData)
        except Exception as e:
            self._log.error(
                f"run_nhl_scores_pipeline: An error occurred while running the NHL scores pipeline: {e}"
            )
            raise Exception(
                f"An error occurred while running the NHL scores pipeline: {e}"
            )
    ...
    def extract_nhl_scores_info(self, url: str):
        try:
            ...
            self._log.error(
                f"extract_nhl_scores_info: An error occurred while extracting NHL scores info: {e}"
            )
    ...
    def transform_nhl_scores_info(self, GameBoxScores: list) -> list:
        try:
            ...
            for gameBoxScore in GameBoxScores:
                ...
                if numberOfGames > 0:
                    for i in range(numberOfGames):
                        ...
                        game_score.home_team = gameBoxScore["games"][i]["homeTeam"][
                            "name"
                        ]["default"]
                        game_score.home_team_image = gameBoxScore["games"][i][
                            "homeTeam"
                        ]["logo"]
                        game_score.away_team = gameBoxScore["games"][i]["awayTeam"][
                            "name"
                        ]["default"]
                        game_score.away_team_image = gameBoxScore["games"][i][
                            "awayTeam"
                        ]["logo"]

                        if "score" in gameBoxScore["games"][i]["homeTeam"]:
                            game_score.home_score = gameBoxScore["games"][i][
                                "homeTeam"
                            ]["score"]
                        else:
                        ...
                        if "score" in gameBoxScore["games"][i]["awayTeam"]:
                            game_score.away_score = gameBoxScore["games"][i][
                                "awayTeam"
                            ]["score"]
                        else:
                        ...
                        if topSeedWins == 0 and bottomSeedWins == 0:
                            ...
                            and topSeedWins < neededToWinSeries
                        ):
                            game_score.series_info = (
                                ""
                                + topSeedName
                            ...
                            and bottomSeedWins == neededToWinSeries
                        ):
                            game_score.series_info = (
                                ""
                                + bottomSeedName
                        ...
                        for goal in range(goalsCount):
                            ...
                            homeTeamAbbrv = gameBoxScore["games"][i]["homeTeam"][
                                "abbrev"
                            ]
                            awayTeamAbbrv = gameBoxScore["games"][i]["awayTeam"][
                                "abbrev"
                            ]
                            if period == 1:
                        ...
                        game_score.final_home_score = gameBoxScore["games"][i][
                            "homeTeam"
                        ]["score"]
                        game_score.final_away_score = gameBoxScore["games"][i][
                            "awayTeam"
                        ]["score"]
            ...
            self._log.error(
                f"transform_nhl_scores_info: An error occurred while transforming NHL scores info: {e}"
            )
            raise Exception(
                f"An error occurred while transforming NHL scores info: {e}"
            )
            ...
            for nhlScoreModel in transformedData:
                ...
                query = """
                    ...
                    VALUES ('{date}', '{home_team}', '{home_team_image}', '{away_team}', '{away_team_image}', {home_score}, {away_score},
                            ...
                            {overtime_away_score}, {final_away_score}, '{round}', {game_number}, '{series_info}');
                """.format(
                    date=nhlScoreModel.game_date,
                    home_team=nhlScoreModel.home_team.replace("'", "''"),
                    home_team_image=nhlScoreModel.home_team_image.replace("'", "''"),
                    away_team=nhlScoreModel.away_team.replace("'", "''"),
                    away_team_image=nhlScoreModel.away_team_image.replace("'", "''"),
                    home_score=nhlScoreModel.home_score,
                    away_score=nhlScoreModel.away_score,
                    first_period_home_score=nhlScoreModel.first_period_home_score,
                    second_period_home_score=nhlScoreModel.second_period_home_score,
                    third_period_home_score=nhlScoreModel.third_period_home_score,
                    overtime_home_score=nhlScoreModel.overtime_home_score,
                    final_home_score=nhlScoreModel.final_home_score,
                    first_period_away_score=nhlScoreModel.first_period_away_score,
                    second_period_away_score=nhlScoreModel.second_period_away_score,
                    third_period_away_score=nhlScoreModel.third_period_away_score,
                    overtime_away_score=nhlScoreModel.overtime_away_score,
                    final_away_score=nhlScoreModel.final_away_score,
                    round=nhlScoreModel.round,
                    game_number=nhlScoreModel.game_number,
                    series_info=nhlScoreModel.series_info.replace("'", "''"),
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert NHL score data for game: {nhlScoreModel.home_team} vs {nhlScoreModel.away_team} on {nhlScoreModel.game_date}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"NHL score data inserted successfully for game: {nhlScoreModel.home_team} vs {nhlScoreModel.away_team} on {nhlScoreModel.game_date}"
                    )
        except Exception as e:
            self._log.error(
                f"load_nhl_scores_info: An error occurred while loading NHL scores info: {e}"
            )
    ...
    def run_nhl_playoff_game_schedules_pipeline(self):
        try:
            ...
            nhlGameSchedules = []
            for nhlGameDate in nhlGameDates:
                jsonData = self.extract_nhl_playoff_game_schedules_info(
                    f"https://api-web.nhle.com/v1/schedule/{nhlGameDate}"
                )
                if jsonData is not None:
                    nhlGameSchedules.append(jsonData)
                else:
                    self._log.warning(
                        f"No NHL game schedules data found for game date: {nhlGameDate}"
                    )
            transformedData: list[nhl_playoff_schedule] = (
                self.transform_nhl_playoff_game_schedules_info(nhlGameSchedules)
            )
            self.load_nhl_playoff_game_schedules_info(transformedData)
        except Exception as e:
            self._log.error(
                f"run_nhl_playoff_game_schedules_pipeline: An error occurred while running the NHL playoff game schedules pipeline: {e}"
            )
            raise Exception(
                f"An error occurred while running the NHL playoff game schedules pipeline: {e}"
            )
    ...
    def extract_nhl_playoff_game_schedules_info(self, url: str):
        try:
            ...
            self._log.error(
                f"extract_nhl_playoff_game_schedules_info: An error occurred while extracting NHL playoff game schedules info: {e}"
            )
            raise Exception(
                f"An error occurred while extracting NHL playoff game schedules info: {e}"
            )

    def transform_nhl_playoff_game_schedules_info(
        self, GameSchedules: list
    ) -> list[nhl_playoff_schedule]:
        try:
                ...
                for counter in range(numberOfGameWeeks):
                    ...
                    if numberOfGamesThisWeek > 0:
                        ...
                        for secondaryCounter in range(numberOfGamesThisWeek):
                            ...
                            if (
                                "score" in games["homeTeam"]
                                and games["homeTeam"]["score"] is not None
                            ):
                            ...
                            if (
                                "score" in games["awayTeam"]
                                and games["awayTeam"]["score"] is not None
                            ):
                            ...
                            playoff_game_schedule_item: nhl_playoff_schedule = (
                                nhl_playoff_schedule(
                                    ...
                                    round,
                                    stationInfo,
                                    int(gameNumberOfSeries),
                                    winningGoaliePlayerID,
                                    winningGoalScorerPlayerID,
                                    venueName,
                                    periods,
                                    series_info,
                                )
                            )
                            transformed_data.append(playoff_game_schedule_item)

            return transformed_data
        except Exception as e:
            self._log.error(
                f"transform_nhl_playoff_game_schedules_info: An error occurred while transforming NHL playoff game schedules info: {e}"
            )
            raise Exception(
                f"An error occurred while transforming NHL playoff game schedules info: {e}"
            )

    def load_nhl_playoff_game_schedules_info(
        self, transformedData: list[nhl_playoff_schedule]
    ):
        try:
            ...
            for nhlGameScheduleModel in transformedData:
                ...
                """.format(
                    gameID=nhlGameScheduleModel.gameID,
                    seasonID=nhlGameScheduleModel.seasonID,
                    game_date=nhlGameScheduleModel.gameDate,
                    start_time=nhlGameScheduleModel.gameStartTime,
                    homeTeamID=nhlGameScheduleModel.homeTeamID,
                    homeTeamScore=nhlGameScheduleModel.homeScore,
                    awayTeamID=nhlGameScheduleModel.awayTeamID,
                    awayTeamScore=nhlGameScheduleModel.awayScore,
                    topSeedsWin=nhlGameScheduleModel.topSeedsWin,
                    topSeedsTeamID=nhlGameScheduleModel.topSeedsTeamID,
                    bottomSeedsWin=nhlGameScheduleModel.bottomSeedsWin,
                    bottomSeedsTeamID=nhlGameScheduleModel.bottomSeedsTeamID,
                    seriesTitle=nhlGameScheduleModel.seriesTitle,
                    round=nhlGameScheduleModel.round,
                    stationInfo=nhlGameScheduleModel.stationInfo,
                    gameNumberOfSeries=nhlGameScheduleModel.gameNumberOfSeries,
                    winningGoaliePlayerID=nhlGameScheduleModel.winningGoaliePlayerID,
                    winningGoalScorerPlayerID=nhlGameScheduleModel.winningGoalScorerPlayerID,
                    venueName=nhlGameScheduleModel.venueName,
                    periods=nhlGameScheduleModel.periods,
                    series_info=nhlGameScheduleModel.seriesInfo.replace("'", "''"),
                )
                result = self._dbManager.execute_query(query)
                if not result:
                    self._log.error(
                        f"Failed to insert NHL playoff schedule data for game: {nhlGameScheduleModel.gameDate}"
                    )
                    exit(1)
                else:
                    self._log.info(
                        f"NHL playoff schedule data inserted successfully for game: {nhlGameScheduleModel.gameDate}"
                    )
        except Exception as e:
            self._log.error(
                f"load_nhl_playoff_game_schedules_info: An error occurred while loading NHL playoff schedule info: {e}"
            )
            raise Exception(
                f"An error occurred while loading NHL playoff schedule info: {e}"
            )
    ...
    def clear_table(self, table_name: str = "seasons") -> bool:
        try:
            ...
            self._log.error(
                f"clear_table: An error occurred while clearing {table_name} table: {e}"
            )
    ...
    def __getTeamPlayerDetails(self, PlayerID: int) -> list:
        try:
            ...
            self._log.error(
                f"__getTeamPlayerDetails: An error occurred while fetching team and player details for player ID {PlayerID}: {e}"
            )
            raise Exception(
                f"An error occurred while fetching team and player details for player ID {PlayerID}: {e}"
            )
